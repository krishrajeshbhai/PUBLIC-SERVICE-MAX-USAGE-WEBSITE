...
async function runTests() {
    ...
    try {
        ...
        if (stopsRes.statusCode !== 200)
            throw new Error(`Stops status was ${stopsRes.statusCode}`);
        ...
        if (searchRes.statusCode !== 201)
            throw new Error(`Search status was ${searchRes.statusCode}`);
        const { options } = searchRes.body.data;
        if (!Array.isArray(options) || options.length === 0)
            throw new Error('Search must return options');
        ...
        if (!fastestOption)
            throw new Error('Could not find fastest option');
        const bookBody = {
            userId: 'user-1',
        ...
        if (bookRes.statusCode !== 201)
            throw new Error(`Book status was ${bookRes.statusCode}`);
        const { ticket, walletBalance } = bookRes.body.data;
        if (!ticket || !ticket.id)
            throw new Error('Booking response missing ticket information');
        ...
        if (bookResDup.statusCode !== 201)
            throw new Error(`Idempotent booking status was ${bookResDup.statusCode}`);
        if (bookResDup.body.data.ticket.id !== ticket.id)
            throw new Error('Idempotent request must return the same ticket');
        ...
        if (walletRes.statusCode !== 200)
            throw new Error(`Wallet status was ${walletRes.statusCode}`);
        ...
        if (liveRes1.statusCode !== 200)
            throw new Error(`Live status 1 was ${liveRes1.statusCode}`);
        ...
        if (delayRes.statusCode !== 201)
            throw new Error(`Delay simulation status was ${delayRes.statusCode}`);
        ...
        if (liveRes2.statusCode !== 200)
            throw new Error(`Live status 2 was ${liveRes2.statusCode}`);
        ...
        if (liveRes2.body.data.status !== 'REROUTED')
            throw new Error("Ticket status should be 'REROUTED'");
        if (!liveRes2.body.data.rerouted)
            throw new Error('Rerouted journey option should be present on ticket');
        ...
        if (crowdRes.statusCode !== 200)
            throw new Error(`Crowd status was ${crowdRes.statusCode}`);
        ...
        if (dashboardRes.statusCode !== 200)
            throw new Error(`Dashboard status was ${dashboardRes.statusCode}`);
        ...
        if (offlineRes.statusCode !== 200)
            throw new Error(`Offline package status was ${offlineRes.statusCode}`);
        ...
        if (chatRes1.statusCode !== 201)
            throw new Error(`Chat status was ${chatRes1.statusCode}`);
        ...
        if (chatRes2.statusCode !== 201)
            throw new Error(`Chat status was ${chatRes2.statusCode}`);
...
await start();
//# sourceMappingURL=verify-nestjs.js.map